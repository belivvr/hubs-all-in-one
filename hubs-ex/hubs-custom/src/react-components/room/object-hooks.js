...
import { AEntity, Static } from "../../bit-components";
...
export function usePinObject(hubChannel, scene, object) {
  const [isPinned, setIsPinned] = useState(isObjectPinned(APP.world, object.el.eid));
  ...
  useEffect(() => {
    ...
    function onPinStateChanged() {
      setIsPinned(isObjectPinned(APP.world, object.el.eid));
    }
...
export function useRemoveObject(hubChannel, scene, object) {
  ...
  const canRemoveObject = !!(
    scene.is("entered") &&
    !isPlayer(object) &&
    !isObjectPinned(APP.world, object.el.eid) &&
    !hasComponent(APP.world, Static, el.eid) &&
